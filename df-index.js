...
const secretKey = "njgkj"

const { MongoClient, ServerApiVersion, ObjectId } = require('mongodb');
const uri = "mongodb+srv://shynidevapriya:shynidf@cluster0.8joykdy.mongodb.net/?appName=Cluster0";
...
async function run() {
  try {
    ...
    const product = client.db("delightsfoods").collection("food")
    const usersCollection = client.db("delightsfoods").collection("users")
    ...
    //       res.send(result)
    //     })
    //  app.get("/sin", async (req, res) => {
    //       const result = await product.find()
    //       const data = await result.toArray()
    //       res.send(data)
    //     })
    //  app.patch("/sing/:id", async (req, res) => {
    //       const id = await req.params.id;
    //       const newid = { _id: new ObjectId(id) }
    //       const data = req.body
    //       const realdata = {
    ...
    //       res.send(result)
    //     })
    //  app.delete("/singlde/:id", async (req, res) => {
    //       const id = await req.params.id;
    //       const newid = { _id: new ObjectId(id) }
    //       const result = await product.deleteOne(newid)
    //       res.send(result)
